...
def is_q_learner(model: PPOModel | QLearner | SARSA) -> TypeGuard[QLearner]:
    return model.model_name == 'QLearner'


def is_ppo(model: PPOModel | QLearner | SARSA) -> TypeGuard[PPOModel]:
    return model.model_name == 'PPO'


def is_sarsa(model: PPOModel | QLearner | SARSA) -> TypeGuard[SARSA]:
    return model.model_name == 'SARSA'


try:
    f = open('SARSA-step.log', 'r')
...
for i in range(start_step, 1000000):
    ...
    while True:
        ...
        output[np].append(
            (ob['players'][np]['score'] - last_score[np]) / 100.0)
    ...
    for np in range(2):
        ...
        v = 0
        if np == winner:
            v += (27 - ob['curr_turn']) / 2 * (1 + ob['players'][np]
    ...
    if i % 10 == 0:
        ...
        print(train_log)
        with open('SARSA.log', 'a+') as log_file:
    ...
    if i % 100 == 0:
        ...
        with open('SARSA-step.log', 'w+') as f:
            f.write(str(i))
    ...
    if ob['curr_turn'] < best_turn:
        ...
        with open('SARSA_pro_play.json', 'w+') as f:
            f.write(json_replay)
