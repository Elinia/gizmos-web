...
for i in range(start_step, 10000000):
    ...
    for np in range(2):
        ...
        if ob["curr_stage"] != Stage.GAME_OVER:
            ...
            v += pow(1.5, 22 - ob["curr_turn"]) + pow(
                1.3, ob["players"][np]["score"] / ob["curr_turn"] * 10 - 12
            )
            # v += 1.0
        else:
            # v -= 20.0 / (ob['players'][np]['score'] + 1) + \
            #     (ob['curr_turn'] - 15) / 10.0
            v -= 4.0 / ((ob["players"][np]["score"] / ob["curr_turn"]) ** 2.5 + 0.05)
            # v -= 1.0
    ...
    if i % 10 == 0:
        raw_log = (
            ...
            p1["score"],
            "return: ",
            "{:.2f}".format(sum(output[0])),
            "{:.2f}".format(sum(output[1])),
        )
    ...
    if (s0 + s1) / turn > best_avg_score:
        ...
        env.save_replay("r/TD3_replay_{}_t{}_{}vs{}.json".format(i, turn, s0, s1))
