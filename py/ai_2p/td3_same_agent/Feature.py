...
class Feature(object):
    ...
    def giz_state(self, np: int, ob: Observation, id: int):
        ...
        belongs_to = (
            "me"
            if giz["belongs_to"] == np
            else "rival"
            if giz["belongs_to"] == 1 - np
            else "None"
        )
        return "{}_{}_{}_{}_{}".format(
            giz["id"], giz["where"], belongs_to, giz["used"], giz["active"]
        )
    ...
    def gen_action_feature(self, action: Action):
        ...
        if (
            act_type == ActionType.BUILD
            or act_type == ActionType.BUILD_FROM_FILED
            or act_type == ActionType.BUILD_FROM_RESEARCH
        ):
