...
if True:
    ...
    from ai_2p.PPOModel_v2 import PPOModel
...
models = [PPOModel(idg, path='ppo-v2-1p.pkl'),
          PPOModel(idg, path='ppo-v2-2p.pkl')]
...
for i in range(start_step, 1000000):
    ...
    if i % 10 == 0:
        ...
        print(train_log)
        with open('PPO-v2.log', 'a+') as log_file:
    ...
    if i % 100 == 0:
        for np in range(2):
            model = models[np]
            model.save('{}-v2-{}p.pkl'.format(model.model_name, np + 1))
        with open('PPO-v2-step.log', 'w+') as f:
            f.write(str(i))
        res = {}
        for i, num in enumerate(build_num):
            res[str(i)] = num
        js = json.dumps(res)
        with open('build_num.json', 'w+') as f:
            f.write(js)

    if i % 10000 == 0:
        for np in range(2):
            model = models[np]
            model.save('{}-v2-{}p{}.pkl'.format(model.model_name, np + 1, i))

    # print("???", torch.sum(models[1].base_embedding))
    if ob['curr_turn'] < best_turn:
        best_turn = ob['curr_turn']
        json_replay = json.dumps(replay)
        with open('PPO-v2_pro_play.json', 'w+') as f:
            f.write(json_replay)
