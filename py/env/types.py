...
class FileFromResearchAction(TypedDict):
    type: Literal[ActionType.FILE_FROM_RESEARCH]
    id: int
...
class BuildFromFileAction(TypedDict):
    type: Literal[ActionType.BUILD_FROM_FILED]
    id: int
    cost_energy_num: dict[Energy, int]
    cost_converter_gizmos_id: list[int]
...
class BuildFromResearchAction(TypedDict):
    type: Literal[ActionType.BUILD_FROM_RESEARCH]
    id: int
    cost_energy_num: dict[Energy, int]
    cost_converter_gizmos_id: list[int]
...
class BuildForFreeAction(TypedDict):
    type: Literal[ActionType.BUILD_FOR_FREE]
    id: int


class ResearchAction(TypedDict):
    type: Literal[ActionType.RESEARCH]
    id: GizmoLevel


class UseGizmoAction(TypedDict):
    type: Literal[ActionType.USE_GIZMO]
    id: int


class GiveUpAction(TypedDict):
    type: Literal[ActionType.GIVE_UP]


class EndAction(TypedDict):
    type: Literal[ActionType.END]
