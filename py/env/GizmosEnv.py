...
class GizmosEnv(Env):
    ...
    def pick_gizmo_from_board(self, id: int):
        ...
        self.state["gizmos_board"][gizmo.level] = [
            *filter(lambda g: g.id != id, self.state["gizmos_board"][gizmo.level]),
            *new_gizmos,
        ]
        return gizmo
    ...
    def avail_actions(self):
        ...
        if self.state["curr_stage"] == Stage.MAIN:
            ...
            if (
                len(self.buildable_gizmos(self.all_board_gizmos, BuildMethod.DIRECTLY))
                > 0
            ):
            ...
            if (
                len(self.curr_player.filed) > 0
                and len(
                    self.buildable_gizmos(
                        self.curr_player.filed, BuildMethod.FROM_FILED
                    )
                )
                > 0
            ):
            ...
            if (
                len(
                    self.buildable_gizmos(
                        self.state["researching"]["gizmos"], BuildMethod.FROM_RESEARCH
                    )
                )
                > 0
            ):
    ...
    def result(self):
        ...
        return list(
            map(
                lambda p: p.index,
                sorted(self.state["players"], key=cmp_to_key(compare)),
            )
        )
    ...
    def build(
        ...
        gizmo = self.pick_gizmo_from_board(id)
        self.curr_player.build(gizmo, cost_energy_num, cost_converter_gizmos_id)
        self.state["curr_stage"] = Stage.TRIGGER
    ...
    def build_from_filed(
        ...
        self.curr_player.build_from_filed(id, cost_energy_num, cost_converter_gizmos_id)
        self.state["curr_stage"] = Stage.TRIGGER
    ...
    def next_player(self):
        if (
            len(self.curr_player.gizmos) >= self.max_gizmos_num
            or len(self.curr_player.level3_gizmos) >= self.max_level3_gizmos_num
        ):
        ...
        self.state["curr_player_index"] = (self.state["curr_player_index"] + 1) % len(
            self.state["players"]
        )
    ...
    def step(self, playerIndex: int, action: Action):
        ...
        try:
            ...
            if not self.action_avail(action["type"]):
                print(
                    "[step] action {} unavailable, stage: {}".format(
    ...
    def observation(self, playerIndex: int | None = None) -> Observation:
        ...
        return {
            ...
            if is_curr_player and self.state["researching"] is not None
            else None,
    ...
    def build_solutions(
        self, gizmo_like: Gizmo | int, method: BuildMethod, check_only: bool = False
    ):
        player = self.curr_player
        gizmo = self.gizmo(gizmo_like) if type(gizmo_like) == int else gizmo_like
    ...
    def all_board_gizmos(self):
        return (
            self.state["gizmos_board"][1]
            + self.state["gizmos_board"][2]
            + self.state["gizmos_board"][3]
        )
    ...
    def space_build(
        self, gizmos: list[Gizmo], method: BuildMethod, action_type: ActionType
    ) -> list[BuildAction | BuildFromFileAction | BuildFromResearchAction]:
        actions = []
        for gizmo in gizmos:
    ...
    def space_build_from_research(self) -> list[BuildFromResearchAction]:
        if self.state["curr_stage"] != Stage.RESEARCH:
            return []
        researching_gizmos = (
            self.state["researching"]["gizmos"] if self.state["researching"] else []
        )
        return self.space_build(
            researching_gizmos,
    ...
    def space_research(self) -> list[ResearchAction]:
        ...
        levels = [1, 2, 3]
        avail_levels = [
            level for level in levels if len(self.state["gizmos_pool"][level]) > 0
        ]
        return [
            {
